...
class ADMM5(abc.ABC): 
    ...
    def __init__(
        ...
        self._max_xi = 0
        self._xi: torch.nn.Parameter = torch.nn.Parameter(
            torch.as_tensor(-1000.0),
            requires_grad=True,
        )

        self.penalty_function: torch.nn.ReLU = torch.nn.ReLU()
        torch_opt = getattr(torch.optim, 'Adam')
        self.xi_optimizer: torch.optim.Optimizer = torch_opt(
            [
                self._xi,
            ],
    ...
    def init_xi(self, xi):
        self._xi.data = torch.as_tensor(xi)

    def compute_xi_loss(self, Lc: float) -> torch.Tensor:
        return self.sigma * self.penalty_function(-self._xi) + self._rho/2 * torch.square(Lc + self._xi + self._lambda/self._rho)
    ...
    def para_update(self, Lc: float) -> None:
        ...
        xi_loss = self.compute_xi_loss(Lc)
        xi_loss.backward()
        self.xi_optimizer.step()
        self._xi.data.clamp_(
            -1000.0,
            self._max_xi,
        )

        delta_lambda = float(self._xi.item() + Lc)
        self._lambda = max(0.0, self._lambda + delta_lambda * self._lambda_lr)
        self._lambda = min(self._lambda, self._penalty_max)

...
class Evaluator:  # pylint: disable=too-many-instance-attributes
    ...
    def evaluate(
        ...
        for episode in track(range(num_episodes), description='Testing...'):
            obs, _ = self._env.reset()
            self._safety_obs = torch.ones(1)
            ep_ret, ep_cost, length = 0.0, 0.0, 0.0
            ...
            # print(f'Episode cost: {ep_cost}')
            # print(f'Episode length: {length}')
        save_replay_path = os.path.join(self._save_dir)
        print('save_replay_path:\t', save_replay_path)
        result_path = os.path.join(save_replay_path, 'result.txt')
        with open(result_path, 'a+', encoding='utf-8') as f:
            print(self._dividing_line, file=f)
            print(f'Evaluation on seed: {seed}', file=f)
            print(f'Average episode reward: {np.mean(episode_rewards)}', file=f)
            print(f'Average episode cost: {np.mean(episode_costs)}', file=f)
            print(f'Average episode length: {np.mean(episode_lengths)}', file=f)
            print(f'Std episode reward: {np.std(episode_rewards)}', file=f)
            print(f'Std episode cost: {np.std(episode_costs)}', file=f)
        print('Saving')
        data = pd.DataFrame(data = np.array([episode_rewards,episode_costs]).T, columns=['Reward', 'Cost'])
        data.to_csv(os.path.join(save_replay_path, f'data_{seed}.csv'))
        return (
            episode_rewards,
            episode_costs,
        )
